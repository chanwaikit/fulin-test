    ...
    window.GLOBAL_VUE.header = new Vue({
        ...
        methods: {
            ...
            handleSearch(id){
                if (id) {
                    if (id === 'all'){
                        window.open(window.GLOBAL_CONFIG.mainUrl+'/productList?country=' + this.search.country,'_self');
                        return
                    }
                    var url = window.GLOBAL_CONFIG.mainUrl+'/productList?c=' + id + '&country='+this.search.country
                    // 拼接分享用户id
                    if(this.search.cId){
                    ...
                    var url2 = window.GLOBAL_CONFIG.mainUrl+'/productList?c=' + this.search.c + '&k=' + this.search.k + '&country=' + this.search.country
                    // 拼接分享用户id
                    if(this.search.cId){
            ...
            toHome(){
                window.open(window.GLOBAL_CONFIG.mainUrl+'/' + this.shareUrl,'_self');
            },
            handleTabClick(tab, event) {
                switch (tab.name) {
                    case 'home':
                        ...
                        break;
                    case 'list':
                        window.open(window.GLOBAL_CONFIG.mainUrl+'/productList' + this.shareUrl,'_self');
                        break;
                    case 'userGuide':
                        window.open(window.GLOBAL_CONFIG.mainUrl+'/userGuide' + this.shareUrl,'_self');
                        break;
                    case 'faq':
                        window.open(window.GLOBAL_CONFIG.mainUrl+'/faq' + this.shareUrl,'_self');
                        break;
                }
            }
        }
    ...
    window.GLOBAL_VUE.orderDialog = new Vue({
        ...
        methods: {
            ...
            getOrderDetails (orderId){
                $.get(window.GLOBAL_CONFIG.mainUrl +'/evaOrder/getOrderDetails/' + orderId,res => {
                    this.amazonForm.amazonOrderNo = res.data.amazonOrderNo
                    this.amazonForm.amazonOrderScreenshot = res.data.amazonOrderScreenshot
                    if(this.amazonForm.amazonOrderScreenshot){
                        this.amazonForm.imageList = [{name:'',url:this.amazonForm.amazonOrderScreenshot}]
                    }
                    this.orderForm.productPicImageUrl = res.data.productImageUrl
                    this.orderForm.productName = res.data.productName
                    this.orderForm.amazonUrl = res.data.amazonUrl
                    this.orderForm.productSku = res.data.productSku
                    this.orderForm.reviewerRemarks = res.data.reviewerRemarks
                    this.orderForm.commentsRemarks = res.data.commentsRemarks
                    this.orderForm.exten.rebates = res.data.exten.rebates
                    this.applyForm.evaOrderId = res.data.id
                    this.amazonForm.id = res.data.id
                    this.time = res.data.exten.time
                    this.orderForm.status = res.data.status
                }, 'json')
            },
            copyKeywordsText() {
            ...
            handleOpen(item,type,isAddOrder){
                if (type === 'keyword'){
                    ...
                    this.handleToAmazon(item.exten.amazonProductUrl,isAddOrder)
                }
            },
...
if(dataPage && seajs.data.alias.hasOwnProperty(dataPage)) {
    seajs.use([dataPage], function (obj) {
        obj && obj.init();
    });
}
// 当网页向下滑动 20px 出现"返回顶部" 按钮
$('.mzt-main').scroll(function(){
    scrollFunction($(this))
})
...
$('[data-mzt-top]').on('click', function () {
    $('.mzt-main').animate({scrollTop:0},500);
})
